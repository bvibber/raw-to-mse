...
class PCMToMP4 {
    ...
    appendSamples(data, timestamp) {
        // hack: only do this at the beginning of the buffer?
        if (this.encoder.samples === 0) {
            let now = this.encoder.samples / this.encoder.sampleRate + this.startTimestamp;
            if (timestamp - now > 0.001) {
                console.log('timestamp', timestamp, 'now', now);
                // Insert silence to avoid discontinuities.
                let samples = Math.floor((timestamp - now) * this.encoder.sampleRate);
                let silence = [];
                for (let i = 0; i < this.encoder.channels; i++) {
                    silence.push(new Float32Array(samples));
                }
                this._reallyAppendSamples(silence, now);
            }
        }
        this._reallyAppendSamples(data, timestamp);
    }

    _reallyAppendSamples(data, timestamp) {
